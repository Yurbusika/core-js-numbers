...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const firstLeg = (x2 - x1) ** 2;
  const secondLeg = (y2 - y1) ** 2;
  const hypotenuse = Math.sqrt(firstLeg + secondLeg);
  return hypotenuse;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const firstVectorLength = Math.hypot(x1, y1);
  const secondVectorLength = Math.hypot(x2, y2);
  return Math.acos(scalarProduct / (firstVectorLength * secondVectorLength));
}
...
function getParallelepipedDiagonal(a, b, c) {
  const squareDiagonal = a ** 2 + b ** 2 + c ** 2;
  return Math.sqrt(squareDiagonal);
}
